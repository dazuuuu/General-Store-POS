...
class BranchModel extends Model
{
    ...
    public function list(): array {$st=$this->db->prepare('SELECT b.*,(SELECT COUNT(*) FROM users u WHERE u.tenant_id=b.tenant_id AND u.branch_id=b.id) staff_count FROM branches b WHERE b.tenant_id=? ORDER BY b.is_default DESC,b.name');$st->execute([\TenantContext::tenantId()]);return$st->fetchAll();}
    ...
    private function ensureDefault(): void {$tid=\TenantContext::tenantId();if(!$tid)return;$st=$this->db->prepare('SELECT id FROM branches WHERE tenant_id=? LIMIT 1');$st->execute([$tid]);if(!$st->fetchColumn())$this->db->prepare("INSERT INTO branches(tenant_id,name,inventory_mode,is_default) VALUES(?,'Main Branch','shared',1)")->execute([$tid]);}
    private function ensureSchema(): void
    {
        $this->db->exec("CREATE TABLE IF NOT EXISTS branches(id INT AUTO_INCREMENT PRIMARY KEY,tenant_id INT NOT NULL,name VARCHAR(160) NOT NULL,location VARCHAR(255) NULL,inventory_mode ENUM('shared','independent') NOT NULL DEFAULT 'shared',is_default TINYINT(1) NOT NULL DEFAULT 0,is_active TINYINT(1) NOT NULL DEFAULT 1,created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,UNIQUE KEY uq_branch_name(tenant_id,name)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
