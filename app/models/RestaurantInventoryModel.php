...
class RestaurantInventoryModel extends Model
{
    ...
    public function restorePortions(int $orderItemId,float $portions,float $linePortions): bool
    {
        if($portions<=0||$linePortions<=0)return false;$tid=\TenantContext::tenantId();$st=$this->db->prepare("SELECT ingredient_id,SUM(CASE WHEN movement_type='sale' THEN -delta_quantity ELSE 0 END) sold_qty,SUM(delta_quantity) net,MAX(unit_cost) unit_cost FROM restaurant_ingredient_movements WHERE tenant_id=? AND order_item_id=? GROUP BY ingredient_id");$st->execute([$tid,$orderItemId]);$rows=$st->fetchAll();if(!$rows)return false;$ins=$this->db->prepare("INSERT INTO restaurant_ingredient_movements(tenant_id,ingredient_id,order_item_id,delta_quantity,unit_cost,movement_type) VALUES(?,?,?,?,?,'return')");foreach($rows as $r){$wanted=round((float)$r['sold_qty']*min(1,$portions/$linePortions),4);$qty=min($wanted,abs(min(0,(float)$r['net'])));if($qty<=0)continue;$this->db->prepare('UPDATE restaurant_ingredients SET quantity=quantity+? WHERE id=? AND tenant_id=?')->execute([$qty,(int)$r['ingredient_id'],$tid]);$ins->execute([$tid,(int)$r['ingredient_id'],$orderItemId,$qty,(float)$r['unit_cost']]);}return true;
    }
