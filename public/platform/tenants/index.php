...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  $db->prepare('UPDATE tenants SET enabled_modules=?,offline_enabled=? WHERE id=?')->execute([json_encode($modules),!empty($_POST['offline_enabled'])?1:0,$id]);
...
<?php foreach($tenants as $tenant):$enabled=json_decode((string)$tenant['enabled_modules'],true);if(!is_array($enabled))$enabled=array_keys(TenantFeatures::MODULES);?>
