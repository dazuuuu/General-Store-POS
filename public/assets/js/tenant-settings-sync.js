(function(w){
  ...
  function removeOffline(){
    if(w.OfflinePOS&&OfflinePOS.revokeOfflineAccess)OfflinePOS.revokeOfflineAccess();
  ...
  function check(){
    ...
    return fetch(config.url,{credentials:'same-origin',cache:'no-store'}).then(function(r){if(!r.ok)throw new Error('settings');return r.json();}).then(function(data){
      if(!data.ok)return;
      if(data.status!=='active'){removeOffline();location.href=config.loginUrl||'/auth/login.php?locked=1';return;}
