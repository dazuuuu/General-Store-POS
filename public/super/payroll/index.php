...
<label class="form-label small mt-2">Sales portion / commission %</label><input name="commission_rate" type="number" min="0" max="100" step=".01" value="0" class="form-control"><div class="form-text">Use salary 0 for commission-only sales agents. Their portion accrues when their product payment is recorded.</div>
...
<?php foreach($employees as $e):?><tr><td class="fw-semibold"><?php echo htmlspecialchars($e['name']);?><?php if($e['login_name']):?><small class="d-block text-muted">POS: <?php echo htmlspecialchars($e['login_name']);?> · <?php echo number_format((float)$e['commission_rate'],2);?>%</small><?php endif;?></td><td><?php echo htmlspecialchars($e['job_title']);?></td><td class="text-end">KES <?php echo number_format((float)$e['salary_amount'],2);?></td><td class="text-center"><?php echo (int)$e['pay_day'];?></td><td class="text-end text-success">KES <?php echo number_format((float)$e['commission_due'],2);?></td><td>
