...
<div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
  <div>
    ...
    <p class="text-muted small mb-0">Set optional wholesale &amp; retail prices for packages and inner items, then move stock into the Store warehouse. From Store you invoice into shop Inventory for POS sales. Prices merge when restocking existing products.</p>
  </div>
  ...
  <div class="card border-0 shadow-sm mb-3" style="border-radius:14px;overflow:hidden;">
    ...
    <div class="p-3">
      ...
      <div class="border rounded mb-3 p-3 transfer-row" style="border-color:#e2e8f0!important;" data-id="<?php echo $id; ?>"
           ...
           data-qty="<?php echo htmlspecialchars((string) $it['quantity']); ?>"
        ...
        <div class="d-flex gap-2 align-items-start mb-2">
          ...
          <div class="flex-grow-1">
            ...
            <div class="text-muted small">
              ...
              · <?php echo $pkgQty > 0 ? htmlspecialchars(rtrim(rtrim(number_format($pkgQty, 2), '0'), '.') . ' ' . $pkgUnit . ' × ' . rtrim(rtrim(number_format($inside, 2), '0'), '.') . ' ' . ($it['unit'] ?: 'piece')) : (rtrim(rtrim(number_format((float) $it['quantity'], 2), '0'), '.') . ' ' . ($it['unit'] ?: 'piece')); ?>
              · Cost <?php echo $pkgBuy > 0 ? ('KES ' . number_format($pkgBuy, 0) . '/' . $pkgUnit) : ('KES ' . number_format($unitBuy, 2) . '/item'); ?>
            </div>
          </div>
        </div>
        <div class="row g-2 transfer-fields" style="opacity:.55;">
          ...
          <div class="col-6 col-md-3">
            <label class="form-label small mb-1">Retail / item inside</label>
          ...
          <div class="col-6 col-md-3">
            <label class="form-label small mb-1">Wholesale / item inside</label>
...
(function () {
  ...
  function rowProfit(row) {
    ...
    var pkgQty = parseFloat(row.dataset.pkgQty) || 0;
    ...
    return { cost: cost, wholesaleReturn: wholesaleReturn, retailReturn: retailReturn };
  }
  ...
  function refreshRow(row) {
    ...
    box.innerHTML = '<strong>Profit preview</strong> · Cost ' + money(p.cost)
  ...
  document.querySelectorAll('.row-check, .f-ws-pack, .f-rt-pack, .f-rt-item, .f-ws-item').forEach(function (el) {
  ...
  document.getElementById('transferForm').addEventListener('submit', function (e) {
    ...
    if (!confirm('Transfer ' + n + ' purchase item(s) into Store warehouse?')) e.preventDefault();
  });
  refreshAll();
})();
</script>
